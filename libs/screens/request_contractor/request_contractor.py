...
class RequestContractor(MDScreen):
    ...
    def add_receiveds(self, req, resulti):
        ...
        for key, received in resulti.items():
            if self.local_id in received['IdLocal']:
                """ significa que essa função e requisição minha"""
                requests = ast.literal_eval(received['requests'])

                for request in requests:
                    showed.append(request)
                    print('Requisição key: ', key)
                    UrlRequest(
                        f'https://obra-7ebd9-default-rtdb.firebaseio.com/Funcionarios/{request}/.json?auth={self.token_id}',
        ...
        for key, request in result.items():
            ...
            if requests:
                if self.local_id in requests:
                    solict.append(self.local_id)
                    """ significa que essa eu enviei uma solicitação para este homen/ser/funcionario/excelente"""
                    print('não aceito')
                    UrlRequest(
                        f"https://obra-7ebd9-default-rtdb.firebaseio.com/Funcionarios/{request['key']}/.json?auth={self.token_id}",
    ...
    def add_requests_decline(self, req, result):
        ...
        for key, decline in result.items():
            ...
            if self.local_id in declines:
                """ significa que essa eu fui recusado pelo funcionario"""
                dc.append(self.local_id)
                UrlRequest(
                    f"https://obra-7ebd9-default-rtdb.firebaseio.com/Funcionarios/{decline['key']}/.json?auth={self.token_id}",
