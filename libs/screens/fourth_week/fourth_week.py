...
class FourthWeek(MDScreen):
    ...
    def upload_days_work_week(self):
        dataframe = ast.literal_eval(self.work_days_week4)
    ...
    def upload_week(self, start_date=None):
        """
        Retorna todas as datas da quarta semana do mês atual (segunda-feira a domingo)
        ...
        # Encontrar a segunda-feira da quarta semana (três semanas depois da primeira segunda)
        fourth_monday = first_monday + timedelta(weeks=3)

        # Último dia do mês
        last_day_of_month = datetime(start_date.year, start_date.month + 1, 1) - timedelta(days=1)

        # Ajustar se a quarta semana ultrapassar o último dia do mês
        if fourth_monday > last_day_of_month:
            fourth_monday = last_day_of_month - timedelta(days=last_day_of_month.weekday())

        # Criar a lista com os dias da semana
        week_days = [(fourth_monday + timedelta(days=i)).strftime("%d/%m/%Y") for i in range(7) if (fourth_monday + timedelta(days=i)) <= last_day_of_month]

        return week_days

    def get_week_dates(self, start_date=None):
        """
        Retorna as datas de início e término da quarta semana do mês atual
        ...
        # Encontrar a segunda-feira da quarta semana
        fourth_monday = first_monday + timedelta(weeks=3)

        # Último dia do mês
        last_day_of_month = datetime(start_date.year, start_date.month + 1, 1) - timedelta(days=1)

        # Ajustar se a quarta segunda ultrapassar o último dia do mês
        if fourth_monday > last_day_of_month:
            fourth_monday = last_day_of_month - timedelta(days=last_day_of_month.weekday())

        # Calcular o domingo correspondente
        fourth_sunday = fourth_monday + timedelta(days=6)
        if fourth_sunday > last_day_of_month:
            fourth_sunday = last_day_of_month  # Ajusta para o último dia do mês, se necessário

        return {
            'start_date': fourth_monday.strftime('%d/%m/%Y'),
            'end_date': fourth_sunday.strftime('%d/%m/%Y'),
            'start_datetime': fourth_monday,
            'end_datetime': fourth_sunday
        }
    ...
    def database_step_one(self):
        ...
        data = {
            'week_4': self.days_work,
            'work_days_week4': f'{week}'
        }
    ...
    def database_step_two(self, req, result, *args):
        ...
        working.week_4 = self.days_work
        working.work_days_week4 = result['work_days_week4']
        # Reset day counters
        self.seg = 0
        self.terc = 0
        self.quart = 0
