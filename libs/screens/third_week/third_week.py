...
class ThirdWeek(MDScreen):
    ...
    def upload_week(self, start_date=None):
        ...
        # Encontrar a segunda-feira da terceira semana (duas semanas depois da primeira segunda)
        ...
        # Ajustar se a terceira semana ultrapassar o último dia do mês
        if third_monday > last_day_of_month:
            third_monday = last_day_of_month - timedelta(days=last_day_of_month.weekday())

        # Criar a lista com os dias da semana
        week_days = [(third_monday + timedelta(days=i)).strftime("%d/%m/%Y") for i in range(7) if (third_monday + timedelta(days=i)) <= last_day_of_month]
    ...
    def get_week_dates(self, start_date=None):
        ...
        # Encontrar a segunda-feira da terceira semana
        third_monday = first_monday + timedelta(weeks=2)

        # Último dia do mês
        last_day_of_month = datetime(start_date.year, start_date.month + 1, 1) - timedelta(days=1)

        # Ajustar se a terceira segunda ultrapassar o último dia do mês
        if third_monday > last_day_of_month:
            third_monday = last_day_of_month - timedelta(days=last_day_of_month.weekday())

        # Calcular o domingo correspondente
        third_sunday = third_monday + timedelta(days=6)
        if third_sunday > last_day_of_month:
            third_sunday = last_day_of_month  # Ajusta para o último dia do mês, se necessário

        return {
            'start_date': third_monday.strftime('%d/%m/%Y'),
            'end_date': third_sunday.strftime('%d/%m/%Y'),
            'start_datetime': third_monday,
            'end_datetime': third_sunday
        }
