...
from kivy.properties import get_color_from_hex, StringProperty, ListProperty, NumericProperty, Clock, BooleanProperty
...
class VacancyContractor(MDScreen):
    ...
    def on_enter(self, *args):
        ...
        print('Tela de vacancy rsrs')
        print('Key recebida: ', self.local_id)
        self.type = 'loc'
    ...
    def on_error(self, req, result):
        print('URL:', req.url)
        print('STATUS:', getattr(req, 'res_status', 'unknown'))
        print('RESULT:', result)
    ...
    def _upload_employee(self, key, key_request):
        ...
        url = f'https://obra-7ebd9-default-rtdb.firebaseio.com/Funcionarios/{key}/.json?auth={self.token_id}'
        try:
            UrlRequest(
                url,
                on_failure=self.on_failure,
                on_error=self.on_failure,
                on_success=lambda req, result, k=key_request: self._next_perfil(req, result, k)
            )
        except:
    ...
    def _next_perfil(self, reqs, info, key, *args):
        print(info)
        ...
        if info:
            ...
            print('Não tem porra de funcionario com o caralhos dessa key:  ',key)

            UrlRequest(
                url=f'https://obra-7ebd9-default-rtdb.firebaseio.com/requets/{key}/.json?auth={self.token_id}',
                method='DELETE',
                on_success=self.snack
            )

    def snack(self, req, result):
        MDSnackbar(
            MDSnackbarText(
                text='A vaga foi removida porque o funcionário associado não está mais disponível',
                theme_text_color='Custom',
                text_color='white',
                bold=True
            ),
