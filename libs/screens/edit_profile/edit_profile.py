...
class EditProfile(MDScreen):
    ...
    def update_ui_from_properties(self):
        ...
        telephone = str(self.telefone)
        print(f'Telefone novo: {telephone}')

        # Atualiza nome
        if "Não definido" not in self.name_user:
            print(self.name_user, ' testando')
            self.ids.name_user.text = self.name_user

        # Atualiza telefone
        if "Não definido" not in telephone:
            print('Telefone não é none')
            self.ids.telefone.text = telephone  # usa a string convertida
    ...
    def on_touch_down(self, touch):
        ...
        if self.keyboard_height > 0:  # Keyboard is open
            fields_focused = any(
                self.ids[field].collide_point(*touch.pos) for field in ['name_user', 'telefone'])
            if not fields_focused:
                for field in ['name_user', 'telefone']:
    ...
    def step_one(self):
        ...
        for field_id, name in [('telefone', 'telefone'), ('name_user', 'nome')]:
        ...
        if (self.ids.telefone.text != self.telefone or
    ...
    def update_database(self):
        ...
        url = f'{self.FIREBASE_URL}/Users/{self.local_id}'
        data = {
    ...
    def error(self, req, result):
        """If the requisition = error"""
        self.show_error('Não foi possivel atualizar os dados')
        Clock.schedule_once(lambda dt: self.show_error('Verifique sua conexão com a internet'), 1.5)
        
    def database_success(self, req, result):
        """"""
        self.show_message('Os dados foram alterados corretamente')
    ...
    def next_page(self, *args):
        ...
        edit_two.token_id = self.token_id
        edit_two.refresh_token = self.refresh_token
        edit_two.local_id = self.local_id
        edit_two.state = self.state
        edit_two.token_id = self.token_id
        edit_two.local_id = self.local_id
        edit_two.refresh_token = self.refresh_token
        edit_two.api_key = self.api_key
