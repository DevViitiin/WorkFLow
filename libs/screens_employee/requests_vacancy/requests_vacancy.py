...
class RequestsVacancy(MDScreen):
    """
    Tela para gerenciar solicitações de vagas enviadas por contratados e recebidas por funcionários.
    Fornece funcionalidades para visualizar, aceitar e gerenciar solicitações de vagas.
    """
    ...
    def on_pre_enter(self, *args):
        ...
        Limpa e prepara para carregar dados frescos do Firebase.
        """
        print('🔄 Preparando tela RequestsVacancy...')
    ...
    def on_enter(self, *args):
        ...
        """
        print(f'Entered RequestsVacancy with key:', self.local_id)
        ...
        self.dialog_not_net = DialogNoNet(
            ...
            callback=lambda: self.signcontroller.retry_signup(self.receiveds)
        )
        
        self.dialog_error_delete = DialogNoNet(
            subtitle='Não foi possível deletar. Verifique sua internet e tente novamente',
            callback=lambda: self.signcontroller.retry_delete(self.upload_requests)
        )
        ...
        # ✅ Sempre limpa e recarrega os dados ao entrar
        self.ids.main_scroll.clear_widgets()
        self._request_count = 0
        ...
        # ✅ Carrega dados baseado na aba atual
        ...
        try:
            ...
            try:
                ...
                for contractor in received:
                    ...
                    contractor_url = f'https://obra-7ebd9-default-rtdb.firebaseio.com/Users/{contractor}/.json?auth={self.token_id}'
    ...
    def handle_contractor_error(self, req, error, key):
        print(error)
    ...
    def upload_requests(self):
        ...
        url = f'{self.FIREBASE_URL}/Functions/.json?auth={self.token_id}'
        UrlRequest(
            url,
            method='GET',
    ...
    def chat(self, *args):
        """Navega para a tela de chat a partir de RequestsVacancy"""
        ...
        # ✅ CORRIGIDO: Atualiza estado e ícones
    ...
    def next_acept(self, name, function, perfil, city, company, email, telephone, state, key):
        ...
        try:
            ...
            accept.city = city
            accept.state = state
            accept.token_id = self.token_id
            accept.refresh_token = self.refresh_token
            accept.local_id = self.local_id
            accept.api_key = self.api_key
        ...
        try:
            ...
            for key, request in requests.items():
                ...
                try:
                    ...
                    if self.local_id in reqs and self.tab_nav_state == 'request':
                        self._request_count += 1
                        self.requests(
                    ...
                    elif self.local_id in decline and self.tab_nav_state == 'decline':
                        self._request_count += 1
                        self.requests(
                            request.get('occupation', 'Vaga não especificada'),
                            'red',
    ...
    def check_firebase_data(self, key, path, callback=None):
        ...
        try:
            url = f'https://obra-7ebd9-default-rtdb.firebaseio.com/{path}/{key}/.json?auth={self.token_id}'
    ...
    def refresh_current_view(self, *args):
        """
