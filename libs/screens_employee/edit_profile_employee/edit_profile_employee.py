...
class EditEmployee(MDScreen):
    ...
    def on_enter(self):
        ...
        try:
            ...
            required_ids = ['name_user', 'perfil', 'telefone', 'function']
    ...
    def check_and_request_permissions(self):
         ...
         if missing_permissions:
             ...
             Clock.schedule_once(lambda dt: self.show_error('Conceda as permissões necessárias'), 2)
    ...
    def step_one(self):
        ...
        try:
            ...
            # Verificar se todos os IDs necessários existem
            required_ids = ['function', 'telefone', 'name_user']
            for id_name in required_ids:
                if not hasattr(self.ids, id_name):
                    logging.error(f"ID '{id_name}' não encontrado na tela")
            ...
            if ( self.ids.telefone.text != self.employee_telephone or
                    self.ids.name_user.text != self.employee_name or
